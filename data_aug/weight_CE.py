...
# 获取类别样本数量
def calculate_class_weights(class_names, dataset):
    class_count = np.zeros(len(class_names))
    for _, label in dataset:
        class_count[label] += 1
...
# 计算训练集中每个类别的权重
class_weights = calculate_class_weights(class_names, new_train_dataset)
...
start_train(num_epochs=50, save_name="weight_CE", model=model, train_loader=new_train_loader, test_loader=test_loader, criterion=criterion, optimizer=optimizer, scheduler=scheduler)